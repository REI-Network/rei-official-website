      ...
      <v-row justify="space-between" class="stake">
        <v-col class="stake-left">
          ...
          <div class="font-grey earn-rewards">
           Earn rewards by voting your $REI to help secure the REI network. <br/>Choose nodes and start earning in just a few clicks.
          </div>
          <v-btn class="stake-btn"> <a href="https://dao.rei.network/#/stake" target="_blank">Vote Now </a></v-btn>
          <ul class="font-grey">
        ...
        <v-col class="stake-right">
          ...
          <v-row class="calculate-input">
              <span class="subheading mr-1 font-grey"
                >Vote</span
              >
              <!-- <span
                class="text-h6 font-weight-light font-amount"
              >{{stake | asset(2)}}</span> -->
              <div style="width:200px;"><v-text-field class="font-blue" style="font-size:24px" v-model="stake" color="#2116E5"></v-text-field></div>
              <span class="subheading font-weight-light mr-1 font-grey"
                >REI</span
              >
          </v-row>
          <!-- <v-slider
          ...
          <v-row class="" justify="space-between">
            <v-col class="text-left">
              <span class="font-weight-light mr-1 font-grey" style="margin-left:8px;">Locking $REI for</span>
              <span
                class="font-blue font-amount"
                v-text="days"></span>
              <span class="font-weight-light mr-1 font-grey">
                days</span>
            </v-col>
          </v-row>
          <v-slider
  ...
  methods: {
    toggle() {
     ...
     async getValidatorDetails(){
       const { data } = await this.$axios.get('https://gateway.rei.network/api/validator/details');
       this.detailsList = data.data;
    },
    async getCalculation(){
            const { data } = await this.$axios.get('https://gateway.rei.network/api/validator')
            this.totalAmount  = data.data.totalAmount;
            this.calculationItems = data.data.activeList;
            this.calculationItems = this.calculationItems.map((item) => {
              let detail = find(this.detailsList,(items) => items.nodeAddress.toUpperCase()  == item.address.toUpperCase());
            ...
            console.log('calculation',this.calculationItems)
            console.log('totalAmount',this.totalAmount)
    },
     Calculation(){
        let votingRewardsYear= 10000000*((parseFloat(this.value.power) + this.stake)/(this.totalAmount + this.stake))*(this.value.commissionRate/100);
        let UserRewards = votingRewardsYear*this.stake/(parseFloat(this.value.power) + this.stake)/365*this.days;
